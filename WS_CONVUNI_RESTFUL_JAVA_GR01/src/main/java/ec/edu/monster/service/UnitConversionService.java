...
public class UnitConversionService {
    ...
    private double toCelsius(double value, String fromUnit) {
        ...
        if ("c".equals(normalizedUnit)) {
            return value;
        }
        if ("f".equals(normalizedUnit)) {
            return (value - 32.0) * 5.0 / 9.0;
        }
        if ("k".equals(normalizedUnit)) {
            return value - 273.15;
        }
        if ("r".equals(normalizedUnit)) {
            return (value - 491.67) * 5.0 / 9.0;
        }
        if ("re".equals(normalizedUnit)) {
    ...
    private String normalizeTemperatureUnit(String unit) {
        ...
        if ("c".equals(normalized)
                || "celsius".equals(normalized)
                || "centigrados".equals(normalized)
                || "centigrado".equals(normalized)
                || "centigrade".equals(normalized)) {
            return "c";
        }
        if ("f".equals(normalized) || "fahrenheit".equals(normalized) || "farenheit".equals(normalized)) {
            return "f";
        }
        if ("k".equals(normalized) || "kelvin".equals(normalized)) {
            return "k";
        }
        if ("r".equals(normalized) || "rankine".equals(normalized)) {
            return "r";
        }
        if ("re".equals(normalized) || "reaumur".equals(normalized) || "reamur".equals(normalized)) {
            return "re";
        }
