...
const defaultBaseUrl = Platform.select({
  android: 'http://192.168.100.158:8080/WS_CONVUNI_RESTFUL_JAVA_GR01/resources/conversion',
  default: 'http://localhost:8080/WS_CONVUNI_RESTFUL_JAVA_GR01/resources/conversion',
}) as string;
...
export function convert(category: ConversionCategory, value: number, fromUnit: string, toUnit: string) {
  const endpoint = `/convert${category.charAt(0).toUpperCase()}${category.slice(1)}`;
  return postJson<ConversionResponse>(endpoint, {
    value,
    fromUnit,
    toUnit,
  });
